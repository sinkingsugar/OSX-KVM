...
qemu-system-x86_64 -enable-kvm -m 14336 -cpu Penryn,kvm=on,vendor=GenuineIntel,+invtsc,vmware-cpuid-freq=on,$MY_OPTIONS\
	  -machine pc-q35-2.9 \
	  -smp 4,cores=2 \
	  -usb -device usb-kbd -device usb-tablet \
	  -device usb-host,vendorid=0x05ac,productid=0x020b -device usb-host,vendorid=0x045e,productid=0x0797 \
	  ...
	  -vga none -device vfio-pci,host=03:00.0,bus=pcie.0,multifunction=on -device vfio-pci,host=03:00.1,bus=pcie.0 \
	  -netdev tap,id=net0,ifname=tap0,script=no,downscript=no -device e1000-82545em,netdev=net0,id=net0,mac=52:54:00:c9:18:27 \
	  -monitor stdio

# -device ide-drive,bus=ide.0,drive=MacDVD \
# -drive id=MacDVD,if=none,snapshot=on,media=cdrom,file=/mnt/data0/ISOs/HighSierra.iso \
